    ...
    <SectionShell anchor={section.anchor} className="wf-band--surface">
      <Container className="wf-page-doc">
        ...
        <p>
          ...
          {config.business.address.postalCode} {config.business.address.city}. Correo de contacto:{' '}
...
const PricingPage: SectionComponent = ({ section }) => {
  ...
  return (
    <SectionShell anchor={section.anchor} className="wf-page">
      <PageHero
        eyebrow="Tarifas"
        title={plans.length > 0 ? 'Planes y paquetes' : 'Paquetes de cuidado'}
      ...
      {packages.length > 0 ? (
        <div className="wf-section wf-band--surface">
          <Container>
            <Heading heading={{ eyebrow: 'Paquetes', title: 'Sesiones completas', subtitle: '' }} />
            <div className="wf-pricegrid">
              {packages.map((item) => (
                <article key={item.id} className={`wf-card wf-pricecard${item.highlighted ? ' is-featured' : ''}`}>
                  {item.highlighted ? <Ribbon label="Más elegido" /> : null}
                  <h3>{item.name}</h3>
                  <p className="wf-price">
                    <strong>{formatPrice(item.priceEur)}</strong>
                  </p>
                  <p className="wf-note">{item.priceNote}</p>
                  <ul className="wf-includes">
                  ...
                  <button type="button" className="wf-btn wf-btn--primary wf-btn--full" onClick={() => elegir(item.id, item.name, 'packageId')}>
                    Elegir paquete
                  </button>
                </article>
              ))}
...
const GalleryPage: SectionComponent = ({ section }) => {
  ...
  return (
    <SectionShell anchor={section.anchor} className="wf-page">
      <PageHero
        eyebrow="Galería"
        title="Transformaciones que enamoran"
        lead="Trabajos reales del salón. Cada perro sale con el manto sano, sin nudos y con un acabado pensado para su raza."
      />
  ...
  return (
    <SectionShell anchor={section.anchor} className="wf-page">
      <PageHero
        ...
        title={`Así trabajamos en ${config.business.name}`}
        ...
        <Container className="wf-storygrid">
          <div className="wf-storygrid__copy">
            ...
            <p className="wf-prose">
              Empezamos como un salón de barrio y hoy atendemos a cientos de familias de {config.business.address.city}.
              Trabajamos sin jaulas, con citas espaciadas para que cada perro tenga su tiempo y con productos adaptados a
              cada tipo de manto y de piel.
            </p>
            <p className="wf-prose">
              Nuestra filosofía es sencilla: si el perro no está tranquilo, paramos. Preferimos dedicar una sesión más a
              acostumbrarle que forzar un acabado perfecto en un animal asustado.
            </p>
          </div>
